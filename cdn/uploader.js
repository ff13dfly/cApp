;(function(){
	...
	var self={
		//需改配置，把外部导入的配置写入
		init:function(con,ids,cfg,agent){
			//1.遍历me，把cfg里有的都放进去
			for(var k in me)if(cfg[k]) me[k]=cfg[k];
		...
		entry:function(con,ids){
			...
			if(ids.length==0){
				...
				me.count=ids.length;
				showCount();			//显示计数器数据
				if(me.getImagesFun!=null){
					me.getImagesFun(ids,function(imgs){		//3.3获取图像的缩略图
						for(var id in imgs){
		...
		row:function(id,src){
			if(src==undefined) return '<li class="'+me.clsThumb+'" id="'+me.imagePre+id+'" style="text-align:center;line-height:96px;"></li>';
			return '<li class="'+me.clsThumb+'" id="'+me.imagePre+id+'" style="text-align:center;line-height:96px;background-image:url('+src+')"></li>';
		},
		thumb:function (id,src){
			$("#"+me.imagePre+id).css({'background-image':'url('+src+')'});
		},
		remove:function(){
			$("#"+me.containerID).find('.'+me.clsThumb).off('click').on('click',function(){
